...
function buildLevel(idx) {
  ...
  const speed = 250 + idx*16;
  const length = 80 + idx*10; // world units
  ...
  const complexity = clamp(idx/14, 0, 1);
  while (x < length - 8) {
    const roll = rnd();
    const wantModeSwitch = idx >= 4 && rnd() < 0.06 + complexity*0.04 && mode === 'cube' && x > 18;
    ...
    if (roll < 0.28) {
      const n = ri(1, 1+Math.floor(complexity*1.4));
      for (let i=0;i<n;i++) obs.push({ x: x+i*0.95, type:'spike' });
      x += n*0.95 + r(3.0, 4.2);
    } else if (roll < 0.5) {
      const w = r(1.5, 2.1 + complexity*0.6);
