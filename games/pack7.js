...
function buildLevel(idx) {
  ...
  const speed = 95 + idx*4;
  const length = 55 + idx*5; // world units
  ...
  const complexity = clamp(idx/40, 0, 1);
  while (x < length - 8) {
    const roll = rnd();
    const wantModeSwitch = idx >= 8 && rnd() < 0.03 + complexity*0.02 && mode === 'cube' && x > 26;
    ...
    if (roll < 0.24) {
      obs.push({ x, type:'spike' });
      x += 1.0 + r(5.0, 6.5);
    } else if (roll < 0.46) {
      const w = r(1.1, 1.5 + complexity*0.4);
  ...
  GAME_GD.init = function(g) {
    ...
    g.updateSelect = function(g, dt) {
      ...
      for (let i=0;i<10;i++) {
        ...
        // only the PLAY button strip at the bottom of the card launches the level —
        // clicking the rest of the card (progress bar, badge, etc.) does nothing,
        // so an accidental tap can't instantly drop you into gameplay
        const playY = cy+ch-24;
        if (p.x>=cx+8 && p.x<=cx+cw-8 && p.y>=playY && p.y<=playY+18) {
    ...
    g.drawSelect = function(g, ctx) {
      ...
      for (let i=0;i<10;i++) {
        ...
        // progress bar
        const barY = cy+52, barW = cw-24;
        ctx.fillStyle = 'rgba(255,255,255,0.08)';
        FX.roundRect(ctx, cx+12, barY, barW, 7, 4); ctx.fill();
        ctx.save();
        ctx.shadowColor = done?'#4ade80':theme[1]; ctx.shadowBlur = 8;
        ctx.fillStyle = done ? '#4ade80' : theme[1];
        FX.roundRect(ctx, cx+12, barY, barW*clamp(best/100,0,1), 7, 4); ctx.fill();
        ctx.restore();
        text(ctx, done ? 'COMPLETE' : best+'% best', cx+cw/2, barY+22, 10.5, done?'#4ade80':'rgba(255,255,255,0.5)', 'center', 600);

        // distinct PLAY button — the only clickable part of the card
        const playY = cy+ch-24;
        ctx.save();
        const btnGrad = ctx.createLinearGradient(cx+8, playY, cx+cw-8, playY);
        btnGrad.addColorStop(0, theme[0]); btnGrad.addColorStop(1, theme[1]);
        ctx.fillStyle = btnGrad;
        ctx.shadowColor = theme[1]; ctx.shadowBlur = 8;
        FX.roundRect(ctx, cx+8, playY, cw-16, 18, 9); ctx.fill();
        ctx.restore();
        text(ctx, 'PLAY', cx+cw/2, playY+13, 11, '#0a0814', 'center', 800);
      }
    ...
    g.startLevel = function(g, idx) {
      ...
      g.flashT = 0;
      g.gravityDir = 1;
      g.introT = 1.1; // shows a "Level X — Get Ready" message before play starts
    };
    ...
    g.updatePlay = function(g, dt) {
      ...
      if (g.introT > 0) {
        g.introT -= dt;
        return; // hold everything still until the intro message clears
      }
      ...
      if (g.mode === 'cube') {
        ...
        if (wantAction && g.grounded) { g.pvy = 14*g.gravityDir; Sound.blip(); FX.burst(g.px*g.PPU, g.groundY, g.theme[1], 8, {speed:80}); }
        g.py += g.pvy*dt;
    ...
    g.drawPlay = function(g, ctx) {
      ...
      if (g.introT > 0) {
        const a = clamp(g.introT/1.1, 0, 1);
        ctx.save();
        ctx.fillStyle = 'rgba(5,4,12,' + (0.55*a+0.15) + ')';
        ctx.fillRect(0,0,g.w,g.h);
        ctx.shadowColor = th[1]; ctx.shadowBlur = 20;
        ctx.fillStyle = '#f3e8ff';
        ctx.font = '700 30px "Space Grotesk", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('LEVEL ' + (g.lvl.seedIdx+1), g.w/2, g.h/2-10);
        ctx.shadowBlur = 0;
        ctx.fillStyle = 'rgba(255,255,255,0.6)';
        ctx.font = '600 13px Inter, sans-serif';
        ctx.fillText('Get ready...', g.w/2, g.h/2+20);
        ctx.restore();
      }
    };
  };
