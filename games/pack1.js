...
const GAME_SNAKE = {
  ...
  update(g, dt) {
    ...
    if (head.x < 0 || head.y < 0 || head.x >= g.cols || head.y >= g.rows) { Sound.bad(); return g.gameOver(); }
    if (g.snake.some(s => s.x === head.x && s.y === head.y))            { Sound.bad(); return g.gameOver(); }
  ...
  draw(g, ctx) {
    ...
    ctx.beginPath();
    ctx.arc(g.food.x*g.cell + g.cell/2, g.food.y*g.cell + g.cell/2, g.cell*0.33, 0, 7);
    ctx.fill();
    g.snake.forEach((s, i) => {
      ctx.fillStyle = i === 0 ? PALETTE.accent2 : PALETTE.accent;
      const p = i === 0 ? 2 : 3;
      ctx.fillRect(s.x*g.cell+p, s.y*g.cell+p, g.cell-p*2, g.cell-p*2);
    });
  }
};
...
const GAME_BREAKOUT = {
  ...
  update(g, dt) {
    ...
    if (g.bricks.every(br => !br.alive)) { Sound.good(); return g.gameOver(true); }
  ...
  draw(g, ctx) {
    ...
    text(ctx, 'LIVES ' + g.lives, g.w-14, 28, 14, PALETTE.dim, 'right');
