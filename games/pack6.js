...
(function(){
  ...
  GAME_FK.init = function(g) {
    ...
    g.drawHUD = function(g, ctx) {
      ...
      grad.addColorStop(0,'rgba(6,10,18,0.75)'); grad.addColorStop(1,'rgba(6,10,18,0.2)');
      ctx.fillStyle = grad;
      ...
      // step tracker: Aim -> Power -> Curve, so the flow is always legible
      const steps = ['AIM','POWER','CURVE'];
      const activeIdx = g.state==='aim'?0 : g.state==='power'?1 : g.state==='curve'?2 : -1;
      if (activeIdx >= 0) {
        const stepW = 64, gap=6, totalW = steps.length*stepW+(steps.length-1)*gap;
        const sx0 = g.w/2 - totalW/2 - 130;
        steps.forEach((s,i) => {
          const bx = sx0 + i*(stepW+gap);
          const isActive = i===activeIdx, isDone = i<activeIdx;
          ctx.save();
          ctx.fillStyle = isActive ? 'rgba(124,58,237,0.35)' : isDone ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.05)';
          FX.roundRect(ctx, bx, 8, stepW, 22, 11); ctx.fill();
          if (isActive) { ctx.strokeStyle='#a855f7'; ctx.lineWidth=1.5; ctx.shadowColor='#a855f7'; ctx.shadowBlur=8; FX.roundRect(ctx, bx, 8, stepW, 22, 11); ctx.stroke(); }
          ctx.restore();
          text(ctx, (isDone?'\u2713 ':'')+s, bx+stepW/2, 23, 9.5, isActive?'#e9d5ff':isDone?'#4ade80':'rgba(255,255,255,0.35)', 'center', 700);
        });
      }
      ...
      FX.roundRect(ctx, g.w/2-58, 46, 116, 22, 11); ctx.stroke();
      ctx.restore();
      text(ctx, 'WIND ' + windDir + ' ' + windMag.toFixed(1), g.w/2, 61, 11, windMag>0.9?'#fbbf24':'rgba(255,255,255,0.5)', 'center', 700);

      if (g.state === 'aim') {
        text(ctx, 'CUSTOMIZE', 16, 44, 9.5, 'rgba(255,255,255,0.35)', 'left', 700);
      ...
      if (g.state === 'power' || g.state === 'curve') {
        ...
        FX.roundRect(ctx, g.w/2-160, barY-20, 320, 52, 14); ctx.fill();
        ctx.strokeStyle = 'rgba(255,255,255,0.1)'; ctx.lineWidth=1;
        FX.roundRect(ctx, g.w/2-160, barY-20, 320, 52, 14); ctx.stroke();
        ctx.restore();
        const val = g.state === 'power' ? g.power : (g.curve+1)/2;
        const label = g.state === 'power' ? 'POWER — click to lock' : 'CURVE — click to lock';
