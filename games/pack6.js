...
const GAME_FK = {
  ...
  init(g) {
    ...
    g.project = function(x, y, z) {
      ...
      const scale = lerp(1, 0.30, Math.min(1,t));
      const groundY = g.h - 66, horizonY = 118;
...
(function(){
  ...
  GAME_FK.init = function(g) {
    ...
    g.ballWorldAt = function(t) {
      const u = t / g.ball.T;
      const x = lerp(g.X0, g.targetX, u) + g.bendMag * Math.sin(u*Math.PI);
    ...
    g.stepFlight = function(g, dt) {
      ...
      if (!g.blocked && pos.z >= g.Z0) {
        if (pos.y > g.BAR_H + 0.05) {
          g.finish(g, 'OVER THE BAR', '#fbbf24');
        } else if (Math.abs(pos.x) > g.GOAL_HALF_W) {
          g.finish(g, 'WIDE', '#fbbf24');
        } else {
    ...
    g.drawGoalAndWall = function(g, ctx) {
      ...
      for (let i=0;i<=8;i++) {
        ...
        ctx.beginPath(); ctx.moveTo(x, barL.y); ctx.lineTo(x, postL.y+2); ctx.stroke();
      }
      for (let i=0;i<=4;i++) {
        const u = i/4;
        const y = lerp(barL.y, postL.y, u);
        ctx.beginPath(); ctx.moveTo(barL.x, y); ctx.lineTo(barR.x, y); ctx.stroke();
      }
      ctx.restore();
      ...
      FX.roundRect(ctx, kp.x-7*kp.scale, kp.y-kh, 14*kp.scale, kh, 4*kp.scale); ctx.fill();
      ctx.fillStyle = '#1e293b';
      ctx.beginPath(); ctx.arc(kp.x, kp.y-kh-5*kp.scale, 5*kp.scale, 0, 7); ctx.fill();
      ctx.restore();
    ...
    g.drawBall = function(g, ctx) {
      ...
      FX.glowCircle(ctx, bp.x, bp.y, 6*bp.scale, '#ffffff', 14);
    ...
    g.drawHUD = function(g, ctx) {
      ...
      text(ctx, '❌ '.repeat(g.misses) + '·'.repeat(g.maxMisses-g.misses), g.w-16, 26, 14, '#f43f5e', 'right');
