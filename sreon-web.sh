...
usage() {
  printf '%s\n' \
    'Sreon Search' \
    '' \
    'bash sreon-web.sh          Start search and open it on your Mac' \
    'bash sreon-web.sh stop     Stop search without deleting its settings' \
    'bash sreon-web.sh status   Show the container status' \
    'bash sreon-web.sh logs     Show application and setup logs' \
    '' \
...
if ! binding="$(compose port sreon 3000)"; then
  printf '%s\n' 'Could not find the search port. Run: bash sreon-web.sh status' >&2
  exit 1
fi
port="${binding##*:}"
case "$port" in
...
until curl --fail --silent --max-time 3 "$url/api/health" | grep -Eq '"connected"[[:space:]]*:[[:space:]]*true'; do
  ...
  if [ "$attempt" -ge 45 ]; then
    printf '%s\n' "The interface is at $url, but the search backend is not ready." 'Run: bash sreon-web.sh status' 'Run: bash sreon-web.sh logs' 'You can try starting again. No search results are fabricated.' >&2
    exit 1
  fi
  sleep 2
done
printf '\nSreon Search is ready: %s\nStop it with: bash sreon-web.sh stop\n' "$url"
