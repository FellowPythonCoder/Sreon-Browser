# Optional web / developer mode

The native Mac app does not use this local-server mode. See README.md for the app.
...
```sh
bash sreon-web.sh
bash sreon-web.sh stop
bash sreon-web.sh status
bash sreon-web.sh logs
```
...
```sh
SREON_NO_OPEN=1 bash sreon-web.sh
```

Use a different port:

```sh
PORT=3001 bash sreon-web.sh
```
...
- **Docker is missing:** Install and initialize Docker Desktop, then run `bash sreon-web.sh` again.
- **Docker does not start:** Open Docker Desktop manually, finish any permission prompts, and retry.
- **Port is busy:** Use `PORT=3001 bash sreon-web.sh`.
- **Backend is not ready:** Run `bash sreon-web.sh status` and `bash sreon-web.sh logs`. Check your internet connection and retry after startup completes.
...
sreon-web.sh        Terminal launcher and container management
