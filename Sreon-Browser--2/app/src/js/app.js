...
(function () {
  ...
  function makeEncryptedStore(storageKeyFor, defaultValue) {
    async function currentKey() {
      return SecureStore.keyFor(activeWorkspaceId);
    }
    async function load() {
  ...
  function normalizeInput(raw) {
    // The address bar displays "Sreon — <query>" for search tabs (see the
    // addr-input assignments below). If the user edits that text instead of
    // retyping from scratch, the literal "Sreon — " prefix comes back to us
    // here as part of the raw input. Strip it before parsing, or it gets
    // treated as part of the query itself and re-added on top of the old
    // one, producing "Sreon — Sreon — <query>" on the next search.
    const value = raw.trim().replace(/^sreon\s*[—-]\s*/i, "");
  ...
  /**
   * Switches into a workspace. Each workspace keeps its own tabs, history,
   * bookmarks and saved logins, encrypted at rest with a device-local key —
   * just no password gate on switching between them anymore.
   */
  ...
  async function renderHistoryList() {
    ...
    el.querySelectorAll(".history-row-main").forEach((row, i) => {
      row.addEventListener("click", () => {
        ...
        Settings.closePanel();
        restoreActiveViewFromOverlay();
        // Load into the tab the user is already on, matching what "history"
        // implies (revisit here) — a fresh tab per click was the bug.
        navigate(entry.url);
      });
    });
